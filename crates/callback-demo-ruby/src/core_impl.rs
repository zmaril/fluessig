//! The hand-written core the generated magnus glue routes into — the seam
...
//! a host callback (the uniform `Box<dyn Fn(i32) + Send + Sync>` the magnus glue
//! builds from a Ruby `Proc`) and returns an unsubscribe closure that removes it;
...
/// it) and the uniform boxed callback the magnus glue built from a Ruby `Proc`.
...
impl TickerCore for TickerImpl {
    ...
    fn tick(&self) {
        ...
        // Fire every live listener under the lock. The demo's Ruby `Proc` only
        // appends to an array, so it never re-enters the ticker (which would
